		...
		if clnt.ClientHdl != nil {
			...
			if bulkInfo != nil && bulkInfo.Count != 0 {
				...
				for i := 0; i < int(bulkInfo.Count); i++ {
					...
					ret_obj.ReqisteredProtocols = string(bulkInfo.BfdSessionStateList[i].ReqisteredProtocols)
					...
					ret_obj.LocalDicriminator = uint32(bulkInfo.BfdSessionStateList[i].LocalDicriminator)
